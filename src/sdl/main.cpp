...
int main(int argc, char** argv)
{
    ...
    ifstream boot9(argv[1], std::ifstream::binary);
    if (!boot9.is_open())
    {
    ...
    ifstream boot11(argv[2], std::ifstream::binary);
    if (!boot11.is_open())
    {
        printf("Failed to open %s\n", argv[2]);
        return 1;
    ...
    ifstream otp(argv[3], std::ifstream::binary);
    if (!otp.is_open())
    {
        printf("Failed to open %s\n", argv[3]);
        return 1;
    ...
    ifstream cid(argv[5], std::ifstream::binary);
    if (!cid.is_open())
    {
        printf("Failed to open %s\n", argv[5]);
        return 1;
